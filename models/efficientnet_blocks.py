...
__all__ = [
    'SqueezeExcite', 'ConvBnAct', 'DepthwiseSeparableConv', 'InvertedResidual', 'InvertedResidual_Share_Act', 'CondConvResidual', 'EdgeResidual']
...
class InvertedResidual_Share_Act(nn.Module):
    """ Inverted residual block w/ optional SE
    ...
    def __init__(
            ...
            noskip=False, exp_ratio=1.0, exp_kernel_size=1, pw_kernel_size=1, act_layer=nn.ReLU,
            norm_layer=nn.BatchNorm2d, se_layer=None, conv_kwargs=None, drop_path_rate=0.):
        super(InvertedResidual_Share_Act, self).__init__()
        conv_kwargs = conv_kwargs or {}
        mid_chs = make_divisible(in_chs * exp_ratio)
        self.has_residual = (in_chs == out_chs and stride == 1) and not noskip
        self.drop_path_rate = drop_path_rate
    ...
    def merge_block(self):
        ...
        self.merged_conv = create_conv2d(self.in_chs, self.out_chs, self.dw_kernel_size, stride=self.stride).cuda()
    ...
    def remove_block(self):
        ...
        if self.stride > 1 or self.in_chs != self.out_chs:
            self.downsample = create_conv2d(self.in_chs, self.out_chs, kernel_size=1, stride=self.stride)
        else:
    ...
    def add_final_act(self, act_name, final_act_lr_scale=1.):
        if not isinstance(self.final_act, Final_Act):
            self.final_act = Final_Act(act_name, final_act_lr_scale)

            if 'learnable' in act_name:
                self.final_act.act_fun.slope.data = self.final_act.act_fun.slope.data.to(self.conv_pw.weight.device)

                # print('self.conv_pw.weight.device:', self.conv_pw.weight.device)
                # print('self.final_act.act_fun.slope.device:', self.final_act.act_fun.slope.device)
