...
from .efficientnet_blocks import SqueezeExcite, InvertedResidual_Share_Act, DepthwiseSeparableConv
...
class EfficientNet(nn.Module):
    ...
    def __init__(self, block_args, num_classes=1000, num_features=1280, in_chans=3, stem_size=32, fix_stem=False,
                 ...
                 se_layer=None, drop_rate=0., drop_path_rate=0., global_pool='avg', config=None):
    ...
    def merge_block(self, slope):
        self.slope = slope

        assert type(slope) == list
        ...
        for module in self.modules():
            if isinstance(module, InvertedResidual_Share_Act):
                if module.act.slope == 1:
                    module.merge_block()
            if isinstance(module, DepthwiseSeparableConv):
                if module.act1.slope == 1:
                    module.act1 = torch.nn.Identity()


    def remove_block(self, slope):
        self.slope = slope
    ...
    def add_final_act(self, slope, act_name, final_act_lr_scale=1.):
        assert type(slope) == list
        ...
        for module in self.modules():
            if isinstance(module, Changable_Act):
                slope_orig.append(module.slope)
                module.slope = slope[cnt]
                cnt += 1
        assert cnt == len(slope)
        ...
        for module in self.modules():
            if isinstance(module, InvertedResidual_Share_Act):
                # print(module.act.slope)
                if module.act.slope == 1:
                    module.add_final_act(act_name, final_act_lr_scale)

        cnt = 0
        for module in self.modules():
            if isinstance(module, Changable_Act):
                if module.slope is not None:
                    module.slope = slope_orig[cnt]
                    cnt += 1
        assert cnt == len(slope_orig)


    def set_stu_handler(self, config):
        assert config.DS.DISTILL
        assert config.DS.ADD_FINAL_ACT

        if hasattr(self, 'handler_list'):
            for handler in self.handler_list:
                handler.remove()

        self.handler_list = []
        self.stu_feature_list = {}

        def forward_hook_stu(module, input, output):
            if output.device not in self.stu_feature_list.keys():
                self.stu_feature_list[output.device] = []

            self.stu_feature_list[output.device].append(output)

        for module in self.modules():
            if isinstance(module, Final_Act):
                self.handler_list.append(module.register_forward_hook(forward_hook_stu))


    def set_tea_handler(self, config, slope):
        assert config.DS.DISTILL
        assert config.DS.ADD_FINAL_ACT
        ...
        self.handler_list = []
        self.tea_feature_list = {}

        self.add_final_act(slope, 'identity')

        def forward_hook_tea(module, input, output):
            if output.device not in self.tea_feature_list.keys():
                self.tea_feature_list[output.device] = []

            self.tea_feature_list[output.device].append(output)

        for module in self.modules():
            if isinstance(module, Final_Act):
                self.handler_list.append(module.register_forward_hook(forward_hook_tea))


    def clear_feature_list(self):
        if hasattr(self, 'stu_feature_list'):
            self.stu_feature_list = {}

        if hasattr(self, 'tea_feature_list'):
            self.tea_feature_list = {}
...
def _gen_mobilenet_v2_block_ds(
        variant, channel_multiplier=1.0, depth_multiplier=1.0, fix_stem_head=False, pretrained=False, **kwargs):
    """ Generate MobileNet-V2 network
    Ref impl: https://github.com/tensorflow/models/blob/master/research/slim/nets/mobilenet/mobilenet_v2.py
    Paper: https://arxiv.org/abs/1801.04381
...
def _gen_efficientnet_lite_block_ds(variant, channel_multiplier=1.0, depth_multiplier=1.0, pretrained=False, **kwargs):
    """Creates an EfficientNet-Lite model.
    ...
    arch_def = [
        ['ds_r1_k3_s1_e1_c16'],
        ['irsa_r2_k3_s2_e6_c24'],
        ['irsa_r2_k5_s2_e6_c40'],
        ['irsa_r3_k3_s2_e6_c80'],
        ['irsa_r3_k5_s1_e6_c112'],
        ['irsa_r4_k5_s2_e6_c192'],
        ['irsa_r1_k3_s1_e6_c320'],
    ]
...
@register_model
def mobilenetv2_100_ds(pretrained=False, **kwargs):
...
@register_model
def mobilenetv2_100_block_ds(pretrained=False, **kwargs):
    """ MobileNet V2 w/ 1.0 channel multiplier """
    model = _gen_mobilenet_v2_block_ds('mobilenetv2_100', 1.0, pretrained=pretrained, **kwargs)
    return model


@register_model
def mobilenetv2_140_ds(pretrained=False, **kwargs):
...
@register_model
def mobilenetv2_140_block_ds(pretrained=False, **kwargs):
    """ MobileNet V2 w/ 1.4 channel multiplier """
    model = _gen_mobilenet_v2_block_ds('mobilenetv2_140', 1.4, pretrained=pretrained, **kwargs)
    return model


@register_model
def mobilenetv2_110d_ds(pretrained=False, **kwargs):
...
@register_model
def mobilenetv2_110d_block_ds(pretrained=False, **kwargs):
    """ MobileNet V2 w/ 1.1 channel, 1.2 depth multipliers"""
    model = _gen_mobilenet_v2_block_ds(
        'mobilenetv2_110d', 1.1, depth_multiplier=1.2, fix_stem_head=True, pretrained=pretrained, **kwargs)
    return model


@register_model
def mobilenetv2_120d_ds(pretrained=False, **kwargs):
...
@register_model
def efficientnet_b0_ds(pretrained=False, **kwargs):
    """ EfficientNet-B0 """
...
@register_model
def efficientnet_b1_ds(pretrained=False, **kwargs):
    """ EfficientNet-B1 """
    # NOTE for train, drop_rate should be 0.2, drop_path_rate should be 0.2
    model = _gen_efficientnet(
...
@register_model
def efficientnet_b2_ds(pretrained=False, **kwargs):
    """ EfficientNet-B2 """
...
@register_model
def efficientnet_b2a_ds(pretrained=False, **kwargs):
    ...
    return efficientnet_b2_ds(pretrained=pretrained, **kwargs)


@register_model
def efficientnet_b3_ds(pretrained=False, **kwargs):
    """ EfficientNet-B3 """
    # NOTE for train, drop_rate should be 0.3, drop_path_rate should be 0.2
    model = _gen_efficientnet(
...
@register_model
def efficientnet_b3a_ds(pretrained=False, **kwargs):
    """ EfficientNet-B3 @ 320x320 w/ 1.0 test crop-pct """
    # WARN this model def is deprecated, different train/test res + test crop handled by default_cfg now
    return efficientnet_b3_ds(pretrained=pretrained, **kwargs)


@register_model
def efficientnet_b4_ds(pretrained=False, **kwargs):
    """ EfficientNet-B4 """
...
@register_model
def efficientnet_b5_ds(pretrained=False, **kwargs):
    """ EfficientNet-B5 """
    # NOTE for train, drop_rate should be 0.4, drop_path_rate should be 0.2
    model = _gen_efficientnet(
...
@register_model
def efficientnet_b6_ds(pretrained=False, **kwargs):
    """ EfficientNet-B6 """
...
@register_model
def efficientnet_b7_ds(pretrained=False, **kwargs):
    """ EfficientNet-B7 """
    # NOTE for train, drop_rate should be 0.5, drop_path_rate should be 0.2
    model = _gen_efficientnet(
...
@register_model
def efficientnet_b8_ds(pretrained=False, **kwargs):
    """ EfficientNet-B8 """
    # NOTE for train, drop_rate should be 0.5, drop_path_rate should be 0.2
    model = _gen_efficientnet(
...
@register_model
def efficientnet_l2_ds(pretrained=False, **kwargs):
    """ EfficientNet-L2."""
    # NOTE for train, drop_rate should be 0.5, drop_path_rate should be 0.2
    model = _gen_efficientnet(
...
@register_model
def efficientnet_lite0_ds(pretrained=False, **kwargs):
    """ EfficientNet-Lite0 """
    # NOTE for train, drop_rate should be 0.2, drop_path_rate should be 0.2
...
@register_model
def efficientnet_lite0_block_ds(pretrained=False, **kwargs):
    """ EfficientNet-Lite0 """
    # NOTE for train, drop_rate should be 0.2, drop_path_rate should be 0.2
    model = _gen_efficientnet_lite_block_ds(
        'efficientnet_lite0', channel_multiplier=1.0, depth_multiplier=1.0, pretrained=pretrained, **kwargs)
    return model


@register_model
def efficientnet_lite1_ds(pretrained=False, **kwargs):
    """ EfficientNet-Lite1 """
    # NOTE for train, drop_rate should be 0.2, drop_path_rate should be 0.2
    model = _gen_efficientnet_lite(
...
@register_model
def efficientnet_lite2_ds(pretrained=False, **kwargs):
    """ EfficientNet-Lite2 """
    # NOTE for train, drop_rate should be 0.3, drop_path_rate should be 0.2
    model = _gen_efficientnet_lite(
...
@register_model
def efficientnet_lite3_ds(pretrained=False, **kwargs):
    """ EfficientNet-Lite3 """
    # NOTE for train, drop_rate should be 0.3, drop_path_rate should be 0.2
    model = _gen_efficientnet_lite(
...
@register_model
def efficientnet_lite4_ds(pretrained=False, **kwargs):
    """ EfficientNet-Lite4 """
    # NOTE for train, drop_rate should be 0.4, drop_path_rate should be 0.2
    model = _gen_efficientnet_lite(
...
@register_model
def tf_efficientnet_lite0_ds(pretrained=False, **kwargs):
    """ EfficientNet-Lite0 """
    # NOTE for train, drop_rate should be 0.2, drop_path_rate should be 0.2
    kwargs['bn_eps'] = BN_EPS_TF_DEFAULT
    kwargs['pad_type'] = 'same'
...
@register_model
def tf_efficientnet_lite1_ds(pretrained=False, **kwargs):
    """ EfficientNet-Lite1 """
    # NOTE for train, drop_rate should be 0.2, drop_path_rate should be 0.2
    kwargs['bn_eps'] = BN_EPS_TF_DEFAULT
    kwargs['pad_type'] = 'same'
...
@register_model
def tf_efficientnet_lite2_ds(pretrained=False, **kwargs):
    """ EfficientNet-Lite2 """
    # NOTE for train, drop_rate should be 0.3, drop_path_rate should be 0.2
    kwargs['bn_eps'] = BN_EPS_TF_DEFAULT
    kwargs['pad_type'] = 'same'
...
@register_model
def tf_efficientnet_lite3_ds(pretrained=False, **kwargs):
    """ EfficientNet-Lite3 """
    # NOTE for train, drop_rate should be 0.3, drop_path_rate should be 0.2
    kwargs['bn_eps'] = BN_EPS_TF_DEFAULT
    kwargs['pad_type'] = 'same'
...
@register_model
def tf_efficientnet_lite4_ds(pretrained=False, **kwargs):
    """ EfficientNet-Lite4 """
    # NOTE for train, drop_rate should be 0.4, drop_path_rate should be 0.2
    kwargs['bn_eps'] = BN_EPS_TF_DEFAULT
    kwargs['pad_type'] = 'same'
...
@register_model
def tf_efficientnet_lite0_block_ds(pretrained=False, **kwargs):
    """ EfficientNet-Lite0 """
    # NOTE for train, drop_rate should be 0.2, drop_path_rate should be 0.2
    kwargs['bn_eps'] = BN_EPS_TF_DEFAULT
    kwargs['pad_type'] = 'same'
...
@register_model
def tf_efficientnet_lite1_block_ds(pretrained=False, **kwargs):
    """ EfficientNet-Lite1 """
    # NOTE for train, drop_rate should be 0.2, drop_path_rate should be 0.2
    kwargs['bn_eps'] = BN_EPS_TF_DEFAULT
    kwargs['pad_type'] = 'same'
...
@register_model
def tf_efficientnet_lite2_block_ds(pretrained=False, **kwargs):
    """ EfficientNet-Lite2 """
    # NOTE for train, drop_rate should be 0.3, drop_path_rate should be 0.2
    kwargs['bn_eps'] = BN_EPS_TF_DEFAULT
    kwargs['pad_type'] = 'same'
...
@register_model
def tf_efficientnet_lite3_block_ds(pretrained=False, **kwargs):
    """ EfficientNet-Lite3 """
    # NOTE for train, drop_rate should be 0.3, drop_path_rate should be 0.2
    kwargs['bn_eps'] = BN_EPS_TF_DEFAULT
    kwargs['pad_type'] = 'same'
...
@register_model
def tf_efficientnet_lite4_block_ds(pretrained=False, **kwargs):
    """ EfficientNet-Lite4 """
    # NOTE for train, drop_rate should be 0.4, drop_path_rate should be 0.2
    kwargs['bn_eps'] = BN_EPS_TF_DEFAULT
    kwargs['pad_type'] = 'same'
